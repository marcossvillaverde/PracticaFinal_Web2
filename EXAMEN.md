...
### Cómo lo arreglé
- Instalé `express-mongo-sanitize` y lo añadí como middleware en `src/app.js` después de `express.json()` y antes de las rutas, para que sanitice automáticamente `req.body`, `req.query` y `req.params`.
...
- `mongoSanitize()` elimina claves que empiezan por `$` o contienen `.` del body/query/params antes de que lleguen a cualquier controlador, bloqueando la inyección NoSQL a nivel de middleware global.
...
2. Si un atacante intenta inyección NoSQL y Mongoose lanza un `MongoServerError`, ese error NO llegaría a Slack. El middleware `error-handler.js` comprueba primero si el error es operacional (`err.isOperational`), luego si es un `ValidationError` de Mongoose, y después si es un duplicado (`err.code === 11000`). Un `MongoServerError` genérico no es operacional ni encaja en los dos casos intermedios, por lo que caería al bloque final (línea 37) donde SÍ se llama a `sendErrorToSlack`. Sin embargo, si el error es un `ValidationError` de Mongoose (que es lo más probable con un objeto en vez de un string), se manejaría en la línea 17 y devolvería un 400 sin notificar a Slack, ya que el middleware lo considera un error controlado.
